...
#include "dukv8/gcobject_pool.h"
#include "dukv8/isolate.h"

namespace v8 {
namespace internal {

GCObjectPool::GCObjectPool() : initialized_(false) {
}

GCObjectPool::~GCObjectPool() {
    if (initialized_) {
        removeList();
    }
}

void GCObjectPool::Initialize() {
    if (!initialized_) {
        initialized_ = true;
        EnsureObjectList();
        addNewList();
...
void GCObjectPool::AddObject(void *val) {
    Initialize();
