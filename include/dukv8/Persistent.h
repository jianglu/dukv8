...
inline Persistent<T> Persistent<T>::New(Handle<T> that) {
    if (that.IsEmpty()) return Persistent<T>();
    T *p = *that;
    return Persistent<T>(reinterpret_cast<T *>(v8::V8::GlobalizeReference(
...
inline void Persistent<T>::Dispose() {
    ...
    v8::V8::DisposeGlobal(reinterpret_cast<internal::Object *>(**this));
}

template<class T>
inline void Persistent<T>::MakeWeak(void *parameters, WeakReferenceCallback callback) {
}

template<class T>
inline void Persistent<T>::ClearWeak() {
}

template<class T>
inline void Persistent<T>::MarkIndependent() {
}

template<class T>
inline void Persistent<T>::SetWrapperClassId(uint16_t class_id) {
}

template<class T>
inline bool Persistent<T>::IsNearDeath() const {
    return false;
}

template<class T>
inline bool Persistent<T>::IsWeak() const {
    return false;
}
